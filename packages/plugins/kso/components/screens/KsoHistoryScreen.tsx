...
export const KsoHistoryScreen: React.FC = () => {
  const { colors } = useTheme();
  const { t } = useTranslation();
  const { settlements, isLoading, error, refresh } = useKsoSettlements();
  ...
  const renderItem = ({ item }: { item: KsoSettlement }) => (
    <View style={[styles.item, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      ...
      <Text style={[styles.itemKsoId, { color: colors.textSecondary }]}>KSO: {item.ksoId}</Text>
      {item.description ? (
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
      <ScreenHeader title={t('kso.history') || 'Riwayat KSO'} />
      {error ? (
...
const styles = StyleSheet.create({
  ...
  itemKsoId: { fontSize: 12, marginTop: 2 },
